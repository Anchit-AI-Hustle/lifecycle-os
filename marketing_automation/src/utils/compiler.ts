import { ThemeContent, FunnelVariant, AdCopyTemplate } from '../types';

// ─────────────────────────────────────────────────────────────────────────────
// compiler.ts — the Campaign Hub landing-page compiler. THEMES x FUNNEL_VARIANTS
// -> a standalone HTML page via compileHTML(theme, variant, baseOrigin).
//
// This file and api/_shared/lp-compiler.js are ONE compiler with two runtimes:
// the serverless port is what /lp/:id actually serves, and this is the same
// content behind the local React workspace. They are kept field-for-field
// identical (only the type annotations differ) so a theme cannot say one thing
// on the deployed page and another in the workspace preview.
//
// Everything below describes KNICKGASM and nothing else: hand-painted one-of-one
// customs on 100% original branded sneakers. Every product name, price and image
// URL comes from the live catalog (data/catalog/products_us.json) and the brand
// record (data/brands/_default.json). Where an approved figure does not exist -
// review counts, star averages - the slot carries a
// [DATA REQUIRED BEFORE LAUNCH: ...] marker rather than a plausible number.
// ─────────────────────────────────────────────────────────────────────────────
const ASSETS = {
...
// Shared image slots. The key NAMES (heroFace / ksmRoot / periSupport / bellyFat
// / tasteAsset) are inherited from the sibling repo this compiler was copied
// from and describe a product line KNICKGASM does not sell. They are rename
// debt, deliberately left alone here: api/_shared/lp-compiler.js and
// src/types.ts read the same slots by name, so renaming them in one file only
// would silently blank every hero image. Rename all three together or not at
// all. Nothing behind them is foreign - every URL is a Knickgasm catalog image
// and none of these names is ever rendered to a viewer.
const THEME_ASSETS = {
...
<head>
  ...
  <title>KNICKGASM Custom Sneakers - ${theme.name}</title>
  <style>
...
/* ─────────────────────────────────────────────────────────────────────────────
 * Paid collateral, DERIVED from the themes above.
 *
 * There is no second table of ad copy. The previous version of this file
 * carried one, and because it was a separate store it kept describing a
 * different business from the landing pages it linked to. Deriving the ad tab
 * from the SAME ThemeContent records means a campaign claim exists in exactly
 * one place: change the theme and the ad, the mailer and the landing page all
 * move with it (docs/campaign-orchestration-master-spec.md 24b, "one record,
 * many views").
 * ───────────────────────────────────────────────────────────────────────────── */

/**
 * The brand's verifiable claims, quoted verbatim from data/brands/_default.json
 * (`claims`) and mirrored in CLAUDE.md's Brand Constants. These are the ONLY
 * things this app may assert as fact. Anything else about the product has to
 * come from a theme record, which in turn comes from the live catalog.
 */
export const VERIFIABLE_CLAIMS: string[] = [
...
/**
 * No approved paid-social audience list exists in this repo. Interest targeting
 * is an account-level artefact that lives in Meta / Google Ads Manager, not in
 * the catalog, so inventing plausible interests here would put an unowned
 * targeting plan in front of a media buyer as though it had been signed off.
 */
export const AD_TARGETING_GAP =
  '[DATA REQUIRED BEFORE LAUNCH: paid-social interest targeting, per theme, US/UK]';

/** Strip the "The Hook:" / "The Craft:" / "The Ask:" prefix off a pointer. */
function pointerBody(pointer: string | undefined): string {
  if (!pointer) return '';
  const at = pointer.indexOf(':');
  return at === -1 ? pointer.trim() : pointer.slice(at + 1).trim();
}

/**
 * Build the Meta / Google collateral for one theme. Every string returned is
 * either copied from the theme record or is one of VERIFIABLE_CLAIMS - nothing
 * is composed out of thin air, and no offer, discount code or review count is
 * introduced that the catalog cannot back.
 *
 * Headlines are the theme's own subject lines. They are written for an email
 * subject, so they can exceed Google's 30-character headline limit; the
 * workspace shows the count so a buyer trims deliberately rather than the tool
 * silently truncating a claim.
 */
export function adCopyForTheme(theme: ThemeContent): AdCopyTemplate {
  const [s1, s2, s3] = theme.subjectLines;
  const hook = pointerBody(theme.mailerPointers[0]);
  const craft = pointerBody(theme.mailerPointers[1]);
  const ask = pointerBody(theme.mailerPointers[2]);

  return {
    themeId: theme.id,
    angleName: theme.name,
    targetInterests: [],           // see AD_TARGETING_GAP
    metaHook: s1 || theme.name,
    metaBody: [hook, craft, ask].filter(Boolean).join('\n\n'),
    metaCTA: 'See The Designs',
    googleHeadline1: s1 || theme.name,
    googleHeadline2: s2 || '',
    googleHeadline3: s3 || '',
    googleDescription1: theme.coreProblem,
    googleDescription2: theme.scientificHook,
    pMaxCallouts: VERIFIABLE_CLAIMS.slice(0, 4),
  };
}

/** The full matrix, one entry per theme. */
export const AD_CAMPAIGN_TEMPLATES: AdCopyTemplate[] = THEMES.map(adCopyForTheme);
