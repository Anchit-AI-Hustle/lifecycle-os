...
```json
{
  "mcpServers": {
    "tencentdb-memory": {
      "command": "node",
      "args": ["/ABSOLUTE/PATH/lifecycle-os/integrations/tencentdb-memory/mcp-server.mjs"],
...
| `TDAI_SESSION` | `claude-default` (repo: `lifecycle-os`) | Default session id when a call omits it. |
