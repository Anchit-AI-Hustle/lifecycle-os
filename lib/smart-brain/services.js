...
class CalendarIntelligenceService {
  ...
  generate({ analysis, competitorBenchmarks, startDate = todayIso(), days = this.config.calendarDays, feedback = [], brand = null }) {
    ...
    const products = analysis.productScores.length ? analysis.productScores : [{ product: { title: 'KNICKGASM Sneaker Assortment', sku: 'KNICKGASM-BUNDLE' }, score: 1 }];
...
function buildEmailAsset(entry, campaignId) {
  ...
  const html = `${meta}<!doctype html><html><head><meta charset="utf-8"><title>${subject}</title></head><body style="margin:0;background:${pal.SURF};color:${pal.INK};font-family:${BODY}"><main style="max-width:680px;margin:auto;background:${pal.SURF}"><section style="background:${pal.P};color:${pal.SURF};padding:36px"><p style="letter-spacing:.16em;text-transform:uppercase;font-family:${BODY};opacity:.85">${bName}</p><h1 style="font-family:${HEAD};font-weight:500">${subject}</h1><p style="font-family:${BODY}">${preheader}</p></section><section style="padding:32px"><h2 style="font-family:${HEAD};color:${pal.P};font-weight:500">${entry.heroProduct.title}</h2><p style="font-family:${BODY}">Crafted around ${entry.objective}, this send connects the creative, landing page, and follow-up logic into one cohort-specific funnel.</p><a href="{{landing_page_url}}" style="background:${pal.P};color:${pal.SURF};padding:14px 20px;text-decoration:none;border-radius:2px;display:inline-block;font-family:${BODY};letter-spacing:1.4px;text-transform:uppercase">${ctaLabel}</a></section></main></body></html>`;
...
function buildAdAssets(entry, campaignId) {
  ...
  const staticAd = (platform, aspect, placement) => ({
    ...
    placement, aspect, headline: safeCopy(hero), primary_text: prodLed, cta: _cta,
