...
function extractAngles(c) {
  ...
  return ['gift', 'streetwear', 'green sneaker', 'kicks', 'dunks', 'detox', 'subscription', 'sale', 'holiday', 'bling kicks'].filter((x) => text.includes(x));
}
...
function buildEmailAsset(entry, campaignId) {
  ...
  const html = `${meta}<!doctype html><html><head><meta charset="utf-8"><title>${subject}</title></head><body style="margin:0;background:#F7F5F2;color:#111111;font-family:${BODY}"><main style="max-width:680px;margin:auto;background:#fff"><section style="background:#6A33D8;color:#F7F5F2;padding:36px"><p style="color:#D0473E;letter-spacing:.16em;text-transform:uppercase;font-family:${BODY}">KNICKGASM</p><h1 style="font-family:${HEAD};font-weight:500">${subject}</h1><p style="font-family:${BODY}">${preheader}</p></section><section style="padding:32px"><h2 style="font-family:${HEAD};color:#6A33D8;font-weight:500">${entry.heroProduct.title}</h2><p style="font-family:${BODY}">Crafted around ${entry.objective}, this send connects the creative, landing page, and follow-up logic into one cohort-specific funnel.</p><a href="{{landing_page_url}}" style="background:#6A33D8;color:#F7F5F2;padding:14px 20px;text-decoration:none;border-radius:2px;display:inline-block;font-family:${BODY};letter-spacing:1.4px;text-transform:uppercase">Shop the edit</a></section></main></body></html>`;
...
function buildAdAssets(entry, campaignId) {
  ...
  const BRAND = 'brand palette only (violet #6A33D8, lava #D0473E, chalk #F7F5F2); real one-of-one packshot; WCAG-AA contrast; NO text baked into the base image (overlays are added in-platform)';
