...
function brandPalette(entry) {
  ...
  const on = (bg, surf) => { try { return core ? core.textOn(bg, surf) : surf; } catch (_) { return surf; } };
  const SURF = p.surface || '#FFFFFF';
  const P = ground(p.primary, p.accent, '#D0473E');
  const A = ground(p.accent, P);
  return { P, A, INK: p.ink || '#111111', SURF, ON_P: on(P, SURF), ON_A: on(A, SURF) };
}
