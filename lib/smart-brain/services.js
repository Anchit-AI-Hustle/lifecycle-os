...
// these is filtered by workspace, and every write is stamped with it. A write
// that is NOT stamped lands with workspace_id NULL and is invisible to the
// scoped policies, so stamping is not optional.
...
class SmartBrainDbAdapter {
  ...
  async ownData() {
    if (this.connected) {
      ...
      const prods = (products && products.length) ? products : this.builtCatalogProducts();
      return { source: 'supabase', products: prods, assets, campaigns, campaignAssets, campaignMetrics, users, orders, events, mvtResults, feedback };
    }
...
class CalendarIntelligenceService {
  ...
  generate({ analysis, competitorBenchmarks, startDate = todayIso(), days = this.config.calendarDays, feedback = [], brand = null }) {
    const entries = [];
      ...
      for (const market of this.config.markets) {
        ...
        for (let k = 0; k < perDay; k++) {
          ...
          const reach = {
            ...
            frequency_cap: { per_rolling_7d: 2, absolute_max: 3, sends_in_rolling_7d: null, over_preferred: false, over_cap: false, status: null, window_complete: null, enforced_at: 'plan-time per-(market,cohort) check + ESP send-time suppression' },
    ...
    // Plan-time promotional frequency cap (spec §10). The contract sets a
    // PREFERRED cap of 2 and an ABSOLUTE cap of 3 promotional sends per rolling
    // 7 days, and going above the preferred cap is allowed "only for a
    // documented high-intent segment + business reason + below absolute +
    // recorded override".
    //
    // This only ever tested `count > absolute_max`, so a cohort planned for
    // exactly 3 promotional sends in 7 days - above the preferred cap, needing a
    // recorded override - came back `over_cap: false` with no status at all, and
    // a reviewer read it as safe. The preferred cap was, in effect, not
    // enforced: only the hard 3 was.
    //
    // Both caps are now evaluated and the slot carries one of the spec's own
    // statuses, so the difference between "safe", "needs a documented override"
    // and "not launch-ready" is on the record instead of inferred.
    //
    // WINDOW EDGE. The count can only see slots inside THIS plan, so a slot in
    // the first six days of the horizon has an incomplete look-back and may
    // undercount sends that already went out before the window opened.
    // `window_complete` says which slots that applies to rather than presenting
    // a partial count as a full one.
    (function enforceFrequencyCap() {
      const dayNum = (d) => Math.floor(Date.parse(`${d}T00:00:00Z`) / 86400000);
      // A send is promotional because it CARRIES A DISCOUNT, not because a
      // code string happens to exist. Keying on the code was safe only while
      // codes were invented for every slot; now that a code comes from the
      // brand's own registry, a brand that has published none would have had
      // every send counted as non-promotional and the cap silently disabled -
      // which is the opposite of what a cap is for. The discount decision is
      // the fact; the code is only how the customer redeems it.
      const isPromo = (e) => !!(e && e.offer && (Number(e.offer.pct) > 0 || e.offer.depth === 'ten' || e.offer.depth === 'fif' || e.offer.code));
      const keyOf = (e) => `${e.market}|${(e.cohort && e.cohort.name) || ''}`;
      const planStart = dayNum(startDate);
      for (const e of entries) {
        ...
        const promo = isPromo(e);
        cap.sends_in_rolling_7d = count;
        cap.window_complete = isFinite(planStart) ? (dn - planStart) >= 6 : null;
        cap.over_preferred = promo && count > cap.per_rolling_7d;
        cap.over_cap = promo && count > cap.absolute_max;
        const who = `"${(e.cohort && e.cohort.name) || 'cohort'}" (${e.market})`;
        if (!promo) {
          cap.status = 'SAFE';
          cap.action = 'Not a promotional send (no offer code), so it does not count toward the cap.';
        } else if (cap.over_cap) {
          cap.status = 'BLOCKED';
          cap.action = `Reduce or delay: ${count} promotional sends to ${who} inside a rolling 7 days exceeds the absolute cap of ${cap.absolute_max}. Absolute-cap fail means NOT LAUNCH READY for this slot.`;
        } else if (cap.over_preferred) {
          cap.status = 'REDUCE_AUDIENCE';
          cap.action = `${count} promotional sends to ${who} inside a rolling 7 days is above the preferred cap of ${cap.per_rolling_7d} and below the absolute cap of ${cap.absolute_max}. Allowed only with a documented high-intent segment, a business reason and a recorded override; without one, reduce the audience or delay the send.`;
        } else {
          cap.status = 'SAFE';
          cap.action = `${count} promotional send(s) to ${who} inside a rolling 7 days is at or under the preferred cap of ${cap.per_rolling_7d}.`;
        }
        if (cap.window_complete === false) {
          cap.action += ' Look-back is partial: this slot sits in the first six days of the plan window, so promotional sends made before the window opened are not counted here.';
        }
      }
    })();
...
function entryBrand(entry) {
  if (entry && entry.brand && entry.brand.id) return entry.brand;
...
function brandPalette(entry) {
  ...
  return { P: p.primary || '#D0473E', A: p.accent || '#6A33D8', INK: p.ink || '#111111', SURF: p.surface || '#FFFFFF' };
}
...
function buildEmailAsset(entry, campaignId) {
  ...
  const subject = safeCopy(`${entry.festival ? `${entry.festival.name}: ` : ''}${heroTitleOf(entry)} for ${entry.cohort.name}`);
  const preheader = safeCopy(`A ${bName} pick built for ${entry.market} ${entry.cohort.name}.`);
  ...
  const body = [
    `<p style="margin:0 0 10px;font-family:${BODY};font-size:11px;letter-spacing:.16em;text-transform:uppercase;color:${pal.SURF}">${bName}</p>`,
    `<h1 style="margin:0 0 12px;font-family:${HEAD};font-size:26px;line-height:1.25;font-weight:500;color:${pal.SURF}">${subject}</h1>`,
    `<p style="margin:0;font-family:${BODY};font-size:15px;line-height:1.5;color:${pal.SURF}">${preheader}</p>`,
  ].join('');
  const main = [
    `<h2 style="margin:0 0 12px;font-family:${HEAD};font-size:20px;line-height:1.3;font-weight:500;color:${pal.P}">${heroTitleOf(entry)}</h2>`,
    `<p style="margin:0 0 24px;font-family:${BODY};font-size:15px;line-height:1.6;color:${pal.INK}">Chosen for ${entry.cohort.name} in ${entry.market}, and carried through the landing page and the follow-up so the promise does not change between them.</p>`,
    `<table role="presentation" border="0" cellpadding="0" cellspacing="0"><tr><td bgcolor="${pal.P}" style="background-color:${pal.P};border-radius:2px"><a href="{{landing_page_url}}" style="display:block;padding:14px 26px;font-family:${BODY};font-size:14px;font-weight:bold;letter-spacing:1.4px;text-transform:uppercase;color:${pal.SURF};text-decoration:none">${ctaLabel}</a></td></tr></table>`,
  ].join('');
...
function buildLandingPageAsset(entry, campaignId) {
  ...
  const mk = (variant, title, blurb, sections) => ({
    ...
    html: (function () {
      ...
      const ctaLabel = entry.cta || 'See more';
      return `<!doctype html><html><head><meta charset="utf-8"><title>${title}</title><meta name="viewport" content="width=device-width,initial-scale=1"></head><body style="margin:0;font-family:Arial,sans-serif;color:${pal.INK};background:${pal.SURF}"><section style="padding:56px 24px;background:${pal.P};color:${pal.SURF};text-align:center"><p style="letter-spacing:.18em;text-transform:uppercase;opacity:.85">${bName}</p><h1>${title}</h1><p>${blurb}</p></section><section style="max-width:920px;margin:auto;padding:40px 24px"><h2>Why this edit</h2><ul><li>Selected from ${kindLine} for ${entry.cohort.name}.</li><li>Proof-led storytelling; reviews render only from approved data.</li><li>Retargeting and mailer follow-up are aligned to this same page promise.</li></ul><a href="${storeHref(entry)}" style="display:inline-block;background:${pal.A};color:${pal.SURF};border:0;padding:16px 24px;font-weight:bold;text-decoration:none;border-radius:2px">${ctaLabel}</a></section></body></html>`;
    })(),
    sections,
  });
  const titleA = safeCopy(`${heroTitleOf(entry)} · ${entry.market} ${entry.cohort.name} Edit`);
...
function buildAdAssets(entry, campaignId) {
  ...
  const prodLed = safeCopy(`${hero}, chosen for ${cohort}. The ${_bName} edit.`);
  const benefitLed = safeCopy(`The ${_kind} ${cohort} keep coming back to. Meet ${hero}.`);
  ...
  const videoAd = (platform, placement) => ({
    ...
    hook: safeCopy(`${cohort}: ${hero} is here`),
    storyboard: [
