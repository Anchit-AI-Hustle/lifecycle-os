...
function alertMessage(items){const lines=items.map((a)=>`${a.severity.toUpperCase()} · ${a.market} · ${a.source} · ${a.metric}: ${a.message}`),rows=items.map((a)=>`<tr><td>${esc(a.severity)}</td><td>${esc(a.market)}</td><td>${esc(a.source)}</td><td><b>${esc(a.metric)}</b><br>${esc(a.message)}</td></tr>`).join('');return{subject:`[KNICKGASM] ${items.some((a)=>a.severity==='critical')?'Critical ':''}Data Analysis anomalies: ${items.length}`,text:`KNICKGASM Lifecycle OS${SENDER()?`\nSender: ${SENDER()}`:''}\n\n${lines.join('\n')}`,html:`<!doctype html><html><body style="font-family:Arial,sans-serif"><h2>KNICKGASM Lifecycle OS anomaly review</h2><table>${rows}</table><p>${SENDER()?`From ${esc(SENDER())} · `:''}automated hourly analysis</p></body></html>`};}
